...
def get_popup_message(jenis_notifikasi, user):
    ...
    messages = {
        ...
        'lulus': (
            ...
            'Silakan melakukan daftar ulang sesuai jadwal dan ketentuan yang telah ditetapkan.<br>'
            'Informasi selengkapnya dapat dilihat pada pengumuman resmi.'
        ),
        'tidak_lulus': (
...
def login():
    if request.method == 'POST':
        ...
        if user and user.check_password(password) and user.role == role:
            ...
            # TAMBAH: Set session untuk notifikasi
            session['show_popup'] = False
            session['popup_message'] = ""
            session['popup_type'] = ""
            
            # Cek jika ada notifikasi belum dibaca
...
def lengkapi_data_siswa():
    ...
    if user.status == 'selesai':
        flash('Pendaftaran Anda sudah diselesaikan. Data tidak dapat diubah lagi.', 'error')
        return redirect(url_for('dashboard_siswa'))
...
def selesaikan_pendaftaran():
    ...
    try:
        ...
        # Log aktivitas
        log_security_event("PENDAFTARAN_SELESAI", request.remote_addr, user.username, 
                          "Pendaftaran diselesaikan")
...
def update_status(user_id):
    ...
    if user and user.role == 'siswa':
        ...
        try:
            ...
            # Jika status berubah ke LULUS, buat notifikasi
            if status == 'lulus' and status_sebelumnya != 'lulus':
                # Cek notifikasi lulus sudah ada belum
            ...
            # Jika status berubah ke TIDAK LULUS, buat notifikasi
